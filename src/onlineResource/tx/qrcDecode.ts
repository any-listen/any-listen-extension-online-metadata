 ...
 *
 * 由原 C++ native 插件（qrc_decode.node）1:1 移植为纯 TS 实现。
...
type TripleDesSchedule = [DesSchedule, DesSchedule, DesSchedule]

// 全部以无符号 32 位处理，>>> 0 保证溢出行为与 C++ uint32_t 一致
const bitnum = (a: Uint8Array, b: number, c: number): number =>
  (((a[((b / 32) | 0) * 4 + 3 - (((b % 32) / 8) | 0)] >>> (7 - (b % 8))) & 1) << c) >>> 0

const bitnum_intr = (a: number, b: number, c: number): number => (((a >>> (31 - b)) & 1) << c) >>> 0

const bitnum_intl = (a: number, b: number, c: number): number => (((((a << b) >>> 0) & 0x80000000) >>> 0) >>> c) >>> 0

const sbox_bit = (a: number): number => (a & 32) | ((a & 31) >>> 1) | ((a & 1) << 4)

// 初始置换：返回 [s0, s1]
const initial_permutation = (input: Uint8Array): [number, number] => {
  const s0 =
    (bitnum(input, 57, 31) |
      bitnum(input, 49, 30) |
      bitnum(input, 41, 29) |
...
const inverse_permutation = (s0: number, s1: number, out: Uint8Array): void => {
  out[3] =
    (bitnum_intr(s1, 7, 7) |
      bitnum_intr(s0, 7, 6) |
      bitnum_intr(s1, 15, 5) |
...
const des_crypt = (input: Uint8Array, schedule: DesSchedule, output: Uint8Array): void => {
...
// 生成 16 轮子密钥
const key_schedule = (key: Uint8Array, mode: typeof DES_ENCRYPT | typeof DES_DECRYPT): DesSchedule => {
...
const tripledes_key_setup = (key: Uint8Array, mode: typeof DES_ENCRYPT | typeof DES_DECRYPT): TripleDesSchedule => {
  if (mode === DES_ENCRYPT) {
    return [
...
// 3DES 加/解密一个 8 字节分组
const tripledes_crypt = (input: Uint8Array, schedule: TripleDesSchedule, output: Uint8Array): void => {
...
// QRC 固定密钥：!@#)(*$%123ZXC!@!@#)(NHL
const QRC_KEY = new Uint8Array([33, 64, 35, 41, 40, 42, 36, 37, 49, 50, 51, 90, 88, 67, 33, 64, 33, 64, 35, 41, 40, 78, 72, 76])

/**
 * 解密腾讯 QRC 歌词。
 * @param hexData 服务端返回的十六进制字符串
 * @returns 解密并解压后的歌词文本（解析失败时返回空字符串）
 */
export const decodeQrc = async (hexData: string): Promise<string> => {
  if (!hexData || hexData.length % 2 !== 0) return ''

  const encrypted = await dataConverter(hexData, 'hex')
  ...
  try {
    const result = await zlib.inflate(encrypted, 'binary', { finishFlush: 'Z_SYNC_FLUSH' })
    return await dataConverter(result, 'utf-8')
  } catch {
    return ''
  }
}
