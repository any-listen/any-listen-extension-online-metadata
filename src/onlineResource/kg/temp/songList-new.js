...
export default {
  ...
  async getListDetail(id, page) {
    ...
    if (/https?:/.test(id)) {
      ...
      if (id.includes('global_collection_id'))
        return this.getUserListDetailByCollectionId(id.replace(/^.*?global_collection_id=(\w+)(?:&.*$|#.*$|$)/, '$1'), page)
      ...
      if (id.includes('.html')) {
        if (id.includes('zlist.html')) {
          ...
          return this.getUserListDetail3(id.replace(/.+\/(\w+).html(?:\?.*|&.*$|#.*$|$)/, '$1'), page)
      }
    ...
    if (codeInfo.userid != null) {
      const songList = await createHttpFetch('http://www2.kugou.kugou.com/apps/kucodeAndShare/app/', {
        ...
        body: {
          ...
          clienttime: 640612895,
          key: '36164c4015e704673c588ee202b9ecb8',
          data: { id: codeInfo.id, type: 3, userid: codeInfo.userid, collect_type: 0, page: 1, pagesize: codeInfo.count },
        },
      })
  ...
  async getUserListDetailByLink({ info }, link) {
    ...
    while (total) {
      ...
      page += 1
      tasks.push(
        createHttpFetch(link.replace(/pagesize=\d+/, 'pagesize=' + limit).replace(/page=\d+/, 'page=' + page), {
          headers: {
            'User-Agent':
              'Mozilla/5.0 (iPhone; CPU iPhone OS 9_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile/13B143 Safari/601.1',
            Referer: link,
  ...
  async getUserListDetail(link, page, retryNum = 0) {
    ...
    if (typeof body == 'string') {
      if (body.includes('"global_collection_id":'))
        return this.getUserListDetailByCollectionId(body.replace(/^[\s\S]+?"global_collection_id":"(\w+)"[\s\S]+?$/, '$1'), page)
    ...
    if (location) {
      ...
      if (location.includes('global_specialid'))
        return this.getUserListDetailByCollectionId(location.replace(/^.*?global_specialid=(\w+)(?:&.*$|#.*$|$)/, '$1'), page)
      if (location.includes('global_collection_id'))
        return this.getUserListDetailByCollectionId(location.replace(/^.*?global_collection_id=(\w+)(?:&.*$|#.*$|$)/, '$1'), page)
      if (location.includes('chain='))
        return this.getUserListDetail3(location.replace(/^.*?chain=(\w+)(?:&.*$|#.*$|$)/, '$1'), page)
